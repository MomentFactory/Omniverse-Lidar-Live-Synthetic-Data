...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        from mf.ov.lidar_live_synth.ogn.OgnBeamToOusterUDPNodeDatabase import OgnBeamToOusterUDPNodeDatabase
        test_file_name = "OgnBeamToOusterUDPNodeTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_mf_ov_lidar_live_synth_BeamToOusterUDPNode")
        database = OgnBeamToOusterUDPNodeDatabase(test_node)
        ...
        expected_value = [-3.14159, 3.14159]
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
        ogts.verify_values(expected_value, db_value, _attr_error(attribute, False))

        self.assertTrue(test_node.get_attribute_exists("inputs:broadcast"))
        attribute = test_node.get_attribute("inputs:broadcast")
        db_value = database.inputs.broadcast
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:ip_address"))
        attribute = test_node.get_attribute("inputs:ip_address")
        db_value = database.inputs.ip_address
